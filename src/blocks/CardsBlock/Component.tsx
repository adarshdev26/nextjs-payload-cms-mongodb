...
export const CardsBlock: React.FC<CardsBlockProps> = ({ heading, cards, cta }) => {
  return (
    <section className="bg-gray-100 py-16 px-4">
      <div className="max-w-7xl mx-auto text-center">
        ...
        <div className="grid gap-8 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
          {cards.map((card, index) => (
            <div
              key={index}
              className="bg-white rounded-3xl shadow-lg p-6 text-center flex flex-col items-center justify-start h-full"
            >
              {card.icon?.url && (
                <img
                  src={card.icon.url}
     ...
     <div className='mt-12'>
      <a href={cta?.link} className='inline-block px-6 py-3 bg-blue-500 text-white rounded-full hover:bg-blue-600' >More Services</a>
     </div>
      </div>
    </section>
  );
